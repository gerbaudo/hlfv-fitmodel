...
for s in args.sca:
	...
	dic["neg"]=float(s.split(":")[3]) #dic["val"]-0.01
	dic["pos"]=float(s.split(":")[4]) #dic["val"]+0.01
	dic["const"]=s.split(":")[5] #"True" #CHANGE HERE
...
if args.ddb:
	...
	#fw.Close()
	dic = {}
	dic["sname"]=args.ddb.split(":")[0] #"%s"%(hname)
	dic["spath"]=args.ddb.split(":")[1] #"%s/%s"%(hpath,hname) if hpath else "%s"%(hname)
	dic["hpath"]=dirname(dic["spath"])
	dic["hname"]=os.path.basename(dic["spath"])
	dic["signal"]=False
	dic["ndd"]="False"
...
#Const="%(const)s"/>\n' % nui)
	if dic["signal"]==True:
		fw.write('    <NormFactor Name="mu_BR_htm" Val="0" Low="-200" High="200" />\n')
	fw.write('  </Sample>\n')
