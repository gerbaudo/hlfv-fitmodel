...
void makeAsimovData(ModelConfig* mcInWs, bool doConditional, RooWorkspace* combWS, RooAbsPdf* combPdf, RooDataSet* combData, int pr_mode)
{

  double mu_val;
  if (pr_mode == 0) mu_val = 0;
  else if (pr_mode == 1) mu_val = 1;
  else mu_val = 1; // temporary
...
////////////////////
  ...
  if (pr_mode == 0 || pr_mode == 1)
  {
    muStr << "_" << mu_val;
  }
  else // only thing else is muhat
  {
    muStr << "_muhat";
  }
...
//save the snapshots of nominal parameters
  ...
  mu->setVal(mu_val);
  if (pr_mode == 0 || pr_mode == 1) mu->setConstant(1);
  else mu->setConstant(0);

  int status = 0;
  if (doConditional)
  {
    cout << "Starting minimization.." << endl;
    RooAbsReal* nll = combPdf->createNLL(*combData, RooFit::Constrain(*mcInWs->GetNuisanceParameters()),RooFit::GlobalObservables(*mcInWs->GetGlobalObservables()));
...
//save the snapshots of conditional parameters
  ...
  if (pr_mode == 2)
  {
    RooArgSet nuisAndPOI(*mcInWs->GetNuisanceParameters(),*mu);
    combWS->saveSnapshot("ucmles",nuisAndPOI);
  }
...
//make the asimov data (snipped from Kyle)
  ...
  cout << "Making asimov data with the following nuis param values:" << endl;
  ...
  if (combWS->data(("asimovData"+muStr.str()).c_str()))
  {
    cout << "Rewriting existing" << endl;
    combWS->import(*asimovData,true);
  }
  else
  {
