...
*/
  ...
  if (!bModel ) {
   ...
   double sig,pval,expsig,exppvalue;
   ...
   if (calcType != 2) { 
      ...
      for (int i = 0; i < 5; ++i) { 
         ...
         if (sig==0) {
	   expsig=htExp.Significance();
	   exppvalue =  htExp.NullPValue();
	 }
      }
   }
   else { 
	 ...
	 if (sig==0) {
	   expsig=ROOT::Math::normal_quantile_c(pval,1);
	   exppvalue =  pval;
	 }
      }
   }

   double obssig = htr->Significance();
   double obspvalue = htr-> NullPValue();

   std::cout << "obssig=" << obssig << "; obspvalue=" << obspvalue << std::endl;
   std::cout << "expsig=" << expsig << "; exppvalue=" << exppvalue << std::endl;

  system(("mkdir -vp " + folder).c_str());
  stringstream ssfileName;
  ssfileName.str("");
  ssfileName << folder << "/output_" << mass << ".root";
  TFile * fw = new TFile(ssfileName.str().c_str(),"UPDATE");
  stringstream smass;
  smass << mass;
  fw->mkdir(smass.str().c_str());
  fw->cd(smass.str().c_str());

  TParameter<Double_t> p1("sig_obs",obssig); p1.Write();
  TParameter<Double_t> p2("sig_med",expsig); p2.Write();
  TParameter<Double_t> p3("pval_obs",obspvalue); p3.Write();
  TParameter<Double_t> p4("pval_med",exppvalue); p4.Write();
