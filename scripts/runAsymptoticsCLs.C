...
//check inputs
  ...
  TFile * fw = new TFile(ssfileName.str().c_str(),"recreate");
  stringstream smass;
  smass << mass;
  fw->mkdir(smass.str().c_str());
  fw->cd(smass.str().c_str());

  TParameter<Double_t> p1("mu_obs_cls",obs_limit); p1.Write();
  TParameter<Double_t> p2("mu_exp_cls",med_limit); p2.Write();
  TParameter<Double_t> p3("mu_2sm_cls",mu_up_n2); p3.Write();
  TParameter<Double_t> p4("mu_1sm_cls",mu_up_n1); p4.Write();
  TParameter<Double_t> p5("mu_1sp_cls",mu_up_p1); p5.Write();
  TParameter<Double_t> p6("mu_2sp_cls",mu_up_p2); p6.Write();
  
  fw->Write();
  fw->Close();
  
  TFile *fwo = TFile::Open("ws_out.root","RECREATE");
  w->Write();
  fwo->Close();
